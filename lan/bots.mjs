...
function decidePro(f) {
  ...
  const giveUp = f.view.phase === "turn" ? 0.068 : (f.view.phase === "river" ? 0.115 : 0);
  ...
  const donkBlock = f.toCall === 0 && f.aggroBehind && eq < 0.90 && Math.random() > 0.055;
...
export function createBotManager(room, broadcast) {
  ...
  function decide(seatId, lv) {
    ...
    const cbet = !pre && view.phase === "flop" && view.toCall === 0 && iRaisedPre &&
                 Math.random() < (live <= 2 ? 0.74 : live === 3 ? 0.58 : 0.40);
