...
function decidePro(f) {
  ...
  const catchBluff = Math.max(0, 0.6 - f.threatCred) * 0.22 +
    ...
    /* ⚠️ 0.075 -> 0.095 เพราะช่องนี้เริ่มเกาะขอบบนหลังใส่ "เคาะเพื่อดูอาการ" (2026-09-04)
       วัด 5 รอบได้ 49.5-59% บนกรอบ 40-58 = หลุดขอบบางรอบ ทำให้ gate แกว่ง
       เหตุผลตรงไปตรงมา: พอเคาะมือกลางแทนที่จะเดิมพัน เวลาโดนยิงใส่มือที่เหลือก็อ่อนลง
       จึงหมอบบ่อยขึ้นตามธรรมชาติ ต้องคืนความกว้างในการสู้ฟลอปกลับไปให้เท่าเดิม */
    (f.live <= 1 && f.view.phase === "flop" ? 0.095 : 0);
...
export function createBotManager(room, broadcast) {
  ...
  function wantsRebuy(b) {
    ...
    /* ⚠️ ตัวเลขชุดนี้คลายขึ้นจากเดิม (2026-09-05 ตามที่เจ้าของสั่ง "ไม่ต้องลุกถี่ เอาสมจริง")
       วัดจากโต๊ะจริง 688 มือ: มีคนลุกทุก 6.1 มือ ทั้งที่เป้าคือทุก ~24
       และรากของมันคือ "หมดตัวแล้วเลิกเลย" เกือบทุกครั้ง — ซึ่งไม่ใช่สิ่งที่คนจริงทำ
       คนที่หมดตักในบ้านเพื่อน ส่วนใหญ่ควักซื้อใหม่ เขาเลิกตอน "หมดจริง ๆ" ไม่ใช่ตอนหมดตัก */
    let want;
    if (b.botLevel === 2) want = 0.96;                       /* นักพนัน แทบไม่เลิก */
    else if (b.botLevel === 1) want = busts >= 3 ? 0.62 : 0.86;  /* มือใหม่ โดนซ้ำ ๆ ถึงจะกลัว */
    else want = busts >= 3 ? 0.74 : 0.92;                    /* มืออาชีพ โต๊ะไม่คุ้มก็ลุก */
    ...
    /* ⚠️ ตัวคูณนี้คือตัวที่ทำให้ลุกถี่เกินจริง
       กระเป๋าเงินอยู่ข้ามคืน มือใหม่กับนักพนันจึงบางลงเรื่อย ๆ ตามธรรมชาติ
       พอบางแล้วตัวคูณเดิมเหลือ 0.15-0.45 = เกือบทุกครั้งที่หมดตัวก็เลิก ตลอดไป
       ตอนนี้มี "พื้นหนี้" คุมปลายทางแล้ว (ติดลบเกินสองเท่าของทุน = เลิกถาวร)
       ตรงนี้จึงใจกว้างได้: ซื้อเข้าใหม่ไปเรื่อย ๆ จนหมดจริง แล้วค่อยหายไปเลย
       ซึ่งตรงกับคนจริงมากกว่า และทำให้คนเล่นได้เจอหน้าเดิมนานพอจะจำกันได้ */
    const runway = purse / buyIn;
    let factor;
    if (runway < 0) factor = 0.30;        /* ต้องกู้มาเล่น มีบ้างแต่ไม่บ่อย */
    else if (runway < 1) factor = 0.55;   /* เหลือไม่พอซื้อเข้าอีกครั้งด้วยซ้ำ */
    else if (runway < 2) factor = 0.80;   /* เหลือครั้งเดียว แต่ก็ยังเล่นต่อ */
    else if (runway < 5) factor = 0.93;   /* เริ่มบาง แต่ยังสู้ไหว */
  ...
  function leaveChance(b) {
    ...
    if (b.botLevel === 1) {
      ...
      /* ⚠️ 0.030 ต่อมือ = ภายใน 33 มือก็ไปแล้ว ซึ่งถี่เกินกว่าคนจริงมาก
         คนที่ได้กำไรแล้วเก็บของกลับ มีจริง แต่เขานั่งต่ออีกเป็นชั่วโมงก่อนจะตัดสินใจ */
      if (up >= 1.6) return 0.012;
      if (up <= 0.35 && (b.busts || 0) >= 1) return 0.010;
      if (m.boredom > 0.6) return 0.006;
      return 0.0005;
    }
    if (b.botLevel === 2) {
    ...
    if (up <= 0.5 && m.tilt > 0.35) return 0.010;
    if (up >= 2.5) return 0.003;
    return 0.0004;
  }
  ...
  /* ⚠️ ห้ามใช้ชื่อ s.leaving เด็ดขาด — poker-room ใช้ชื่อนั้นไปแล้วคนละความหมาย
     ของห้องแปลว่า "คนกดออกกลางมือทั้งที่ยังมีเงินในกอง" แล้วพอจบมือห้องจะ cashOut ทิ้งเอง
     ตอนแรกผมตั้งชื่อชนกันเข้า ผลคือบอทที่ติดป้ายว่าจะลุก ถูกห้องเก็บออกไปก่อน finishLeaving
     ตัวแทนจึงไม่ถูกเรียกมานั่ง โต๊ะค่อย ๆ ว่างลง และ "ลุกจากโต๊ะ" นับได้ 0 ครั้งใน 5,100,000 มือ
     ทั้งที่โค้ดถูกเรียก 411 ครั้งต่อ 12,000 มือ — บั๊กชื่อชนกันที่เงียบสนิท */
  const LEAVE_MS = 2200;

  function botLeaves(b) {
    if (b.botBye) return;                 /* สั่งซ้ำระหว่างที่ยังโบกมืออยู่ ไม่ต้องทำอะไร */
    b.botBye = Date.now() + LEAVE_MS;
    /* ⚠️ นาฬิกาอย่างเดียวใช้ไม่ได้ — เครื่องมือวัดเดินเป็นล้านมือในไม่กี่นาที
       2.2 วินาทีจึงไม่มีวันผ่านในนั้น แล้ว "บอทลุกจากโต๊ะ" ก็ตายสนิทในทุกการวัด
       เจอจริงหลังรัน 5,100,000 มือ: ทั้ง 30 ตัว ลุก 0 ครั้ง ทั้งที่ล้มกันคนละ 2-13 ครั้งต่อพันมือ
       (บั๊กตระกูลเดียวกับช่วงเว้นคำพูดที่เพิ่งแก้ไปเมื่อกี้ — เขียนเป็นวินาทีในที่ที่เวลาไม่เดิน)
       ใส่เส้นตายเป็น "จำนวนมือ" ควบไปด้วย ครบอันไหนก่อนก็จบ
       ในเกมจริงมือหนึ่งใช้เวลาเป็นนาที เส้นเวลาจึงมาก่อนเสมอ — ภาพลายังยาว 2.2 วิเท่าเดิม */
    b.byeHand = room.table._state.handNo + 2;
    ...
    broadcast();
    /* ⚠️ เส้นตายมีสองเส้น เพราะเครื่องมือวัดเดินเป็นล้านมือในไม่กี่นาที
     2.2 วินาทีจึงไม่มีวันผ่านในนั้น แล้ว "ลุกจากโต๊ะ" ก็ตายสนิทในทุกการวัด
     (บั๊กตระกูลเดียวกับช่วงเว้นคำพูด — เขียนเป็นวินาทีในที่ที่เวลาไม่เดิน)
     ครบเส้นไหนก่อนก็จบ · ในเกมจริงมือหนึ่งใช้เวลาเป็นนาที เส้นเวลาจึงมาก่อนเสมอ
     ภาพลาบนจอยังยาว 2.2 วิเท่าเดิม */
  ...
  function settleBusted() {
    ...
    for (const b of botSeats()) {
      ...
      if (b.botBye) {
        if (Date.now() >= b.botBye ||
            (b.byeHand !== undefined && st.handNo >= b.byeHand)) finishLeaving(b);
        continue;
      }
