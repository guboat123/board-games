...
const LEVEL = {
  ...
  3: { name: "มืออาชีพ",     margin:  0.036, preMargin: 0.095, bet: 0.44, raise: 0.58, preRaise: 0.415, bluff: 0.16, think: [1100, 3400], sizing: 0.60 }
};
...
function decidePro(f) {
  ...
  const catchBluff = Math.max(0, 0.6 - f.threatCred) * 0.22 +
    ...
    (f.live <= 1 && f.view.phase === "flop" ? 0.045 : 0);
  ...
  const openGap = f.pre ? (f.seatsLeft >= 6 ? 0.10
                         : f.seatsLeft >= 4 ? 0.06
                         : f.seatsLeft >= 2 ? 0.02
                         : -0.05) : 0;
  ...
  if (f.toCall === 0) {
    ...
    if (f.aggroBehind && eq < 0.90 && Math.random() > 0.010) {
      return { type: "act", action: "check" };
    }
