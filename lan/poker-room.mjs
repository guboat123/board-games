...
export function createTable(opts = {}) {
  ...
  /* ⚠️ ต่อให้ตั้ง "ไม่จำกัดเวลา" ก็ต้องมีเพดานสุดท้ายเสมอ
     โต๊ะที่ turnSeconds = 0 แล้วคนที่ถึงตาเงียบไปเฉยๆ (ยังต่อเน็ตอยู่ แค่ไม่กด) จะค้างถาวร
     ไม่มีทางออกเลย: กด endrun ก็ไม่ได้ ("มือนี้ยังไม่จบ") คนอื่นก็หมอบแทนไม่ได้
     ต้องลุกออกกันทั้งโต๊ะ (ทดสอบแล้วค้างเกิน 47 วินาทีโดยไม่มีที่สิ้นสุด)
     "ไม่จำกัดเวลา" หมายถึง "ไม่มีนาฬิกาให้กดดัน" ไม่ใช่ "รอตลอดกาล" */
  const HARD_TURN_CAP_MS = 180000;   /* 3 นาที */

  function tick(now) {
    now = now || Date.now();
    const budget = turnBudget() || HARD_TURN_CAP_MS;
    if (st.current < 0 || !st.turnAt) return false;
