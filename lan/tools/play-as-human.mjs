/* ===========================================================
   ...
   รัน:  node lan/tools/play-as-human.mjs [จำนวนมือรวม] [มือต่อวง] [ระดับ]
   ระดับ: เว้นว่าง = คละ 3/3/2 · "1" = มือใหม่ล้วน · "2" = นักพนันล้วน · "3" = มืออาชีพล้วน
   ⚠️ ตัวเลข "ได้กี่บิ๊กบลายด์ต่อ 100 มือ" ขึ้นกับส่วนผสมของโต๊ะอย่างมาก
   โต๊ะคละมีคนเล่นหลวม 6 จาก 8 ซึ่งอ่อนกว่าโต๊ะจริงมาก เอาไปเทียบกับเลขมาตรฐานตรง ๆ ไม่ได้
   เทียบกับ "มืออาชีพล้วน" ต่างหากที่บอกได้ว่าระดับยากสุดเล่นเป็นจริงหรือเปล่า
   =========================================================== */
...
const ONLY = String(process.argv[4] || "").trim();
const PER_LEVEL = ONLY === "1" ? { 1: 8, 2: 0, 3: 0 }
                : ONLY === "2" ? { 1: 0, 2: 8, 3: 0 }
                : ONLY === "3" ? { 1: 0, 2: 0, 3: 8 }
                : { 1: 3, 2: 3, 3: 2 };
...
console.log("ผมนั่งเล่นเอง " + handsPlayed.toLocaleString("en-US") + " มือ · " +
            (ONLY ? (LVNAME[ONLY] || ONLY) + "ล้วน · " : "โต๊ะคละระดับ · ") +
